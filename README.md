...
**Layered / N-Tier.** `Controller → Service → Repository`, anemic model, one shared `DbContext` — the most widely deployed shape in .NET, and the baseline every variant above is reacting against. It's left out because [`Modular.Monolith/`](Modular.Monolith/README.md) already holds the single-process slot, and N-Tier's defining property is the *absence* of boundaries between contexts. Adding it would mean a sixth variant whose only contribution is the fifth one minus its `internal` visibility rules. The comparison worth having — enforced module boundaries against none — is already there for anyone who reads the monolith and pictures it without them.

**Event-Driven (choreography + sagas).** The strongest remaining gap, and an honest one. Every multi-process variant here runs over RabbitMQ, but `IHikyaku.Send` is request/response — synchronous semantics on an asynchronous transport. The real thing is a different animal: a ride-lifecycle saga with compensations, a transactional outbox, idempotent consumers, and `RequestRide` answering `202 Accepted` with the outcome arriving later over SignalR, rather than a fully-populated ride in the response body.

It isn't here because the .NET saga ecosystem is mid-consolidation and this repo has already paid that bill once. MassTransit v9 is commercially licensed and the Apache 2.0 v8 line is supported only through the end of 2026 — adopting it would schedule a repeat of the Arbitrer migration that produced [the Hikyaku move below](#cross-service-communication). Wolverine and Rebus (both MIT) are the credible alternatives, and one of them will carry this variant if it lands. Picking a message-bus framework is the highest-consequence dependency decision in the portfolio, and it was worth not making it in a hurry.

For browsing every variant side by side in a single IDE window, open [`Voyager.All.sln`](Voyager.All.sln) at the repo root — an aggregate solution containing all 87 projects, grouped into solution folders that mirror the on-disk layout. It's an IDE convenience only: each variant's own `.sln` stays the canonical build unit, and CI builds those five individually.
