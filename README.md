...
| [`Hexagonal.Architecture/`](Hexagonal.Architecture/README.md) | Hexagonal (Ports & Adapters) | One core per service (entities + primary/secondary ports + use cases); every use case is reachable both by direct local injection and by remote Arbitrer dispatch through the same primary-port interface, no `IMediator.Send` in controllers. The cost of that dual reachability: the primary ports are declared as `IRequestHandler<T>` (`IAcceptRideUseCase : IRequestHandler<AcceptRide>`), so the hexagon's driving edge is expressed in a MediatR type and `{Service}.Core` carries a MediatR package reference | ✅ done |
