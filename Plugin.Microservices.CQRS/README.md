...
### Cache-Aside
Driver status wraps DB access in `cache.GetOrCreateAsync(...)` with a short TTL, backed by Redis, and the write paths invalidate the key. See `Driver.Handlers/CQRS/Queries/GetDriverStatusHandler.cs` for the read side and `UpdateLocationHandler.cs` / `UpdateAvailabilityHandler.cs` for the eviction. It is the only cached read in the variant: driver search is deliberately uncached (see `DESIGN.md`) and ratings go straight to SQL on every call.
