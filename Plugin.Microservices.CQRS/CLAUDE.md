...
- Three things about the harness are load-bearing:
  ...
  - **`ResetDatabaseAsync` truncates via Respawn, it does not recreate.** `EnsureDeleted` + `EnsureCreated` rebuilds the schema from the EF model, which drops everything a migration does outside the model — `SIX_Drivers_LastLocation` (raw SQL in `20260731180239_AddDriverLastLocationSpatialIndex`) among it. `DatabaseResetTests` pins this.
