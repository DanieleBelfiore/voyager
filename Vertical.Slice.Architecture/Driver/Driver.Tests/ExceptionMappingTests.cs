...
/// <summary>
/// Covers Commons/Voyager.Shared's exception-to-status table. It lives in Driver.Tests for the
/// same reason RedisCacheServiceTests does: Voyager.Shared has no test project of its own, and
/// Driver.Api is the reference that makes it reachable. The file under test is the single copy
/// shared by Clean, Hexagonal, Vertical Slice and Modular Monolith, so asserting it once covers
/// all four — Plugin.Microservices.CQRS keeps its own copy and its own test.
/// </summary>
public class ExceptionMappingTests
{
  ...
  {
    { new ValidationException([new ValidationFailure("Rating", "out_of_range")]), StatusCodes.Status400BadRequest, "validation_failed" },
    ...
    { new KeyNotFoundException("ride_not_found"), StatusCodes.Status404NotFound, "not_found" },
