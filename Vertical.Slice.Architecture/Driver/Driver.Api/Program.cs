...
// No repository, no DbContext-behind-an-interface: every feature's handler injects
// DriverDbContext directly. There is no Infrastructure project to isolate it in — the point of
// this variant is that a feature takes exactly the dependency it needs, in its own folder.
builder.Services.AddDbContext<DriverDbContext>((provider, options) =>
{
...
builder.Services.Configure<MatchingWeights>(configuration);
