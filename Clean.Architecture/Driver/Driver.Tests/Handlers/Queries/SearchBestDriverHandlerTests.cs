...
public class SearchBestDriverHandlerTests
{
  ...
  public SearchBestDriverHandlerTests()
  {
    ...
    // Cache always misses straight through to the factory - the handler's own logic is what's under test here.
    ...
    _handler = new SearchBestDriverHandler(_repository, _ratingsQuery, _cache, _weights);
  }
  ...
  public async Task Handle_ReturnsEmpty_WhenNoDriverWithinThreshold()
  {
    ...
    // Act
    var result = await _handler.Handle(new SearchBestDriver { UserId = Guid.NewGuid(), Location = new Point(0, 0), DistanceThresholdInMeters = 10 }, CancellationToken.None);
