...
`{Service}.Domain` → referenced by nothing, references nothing but NetTopologySuite where relevant. (Hub has none — don't create one just to match the other services' shape.) One exception: `Ride.Domain` also references `Commons/Voyager.Errors`, two dependency-free exception types its entity invariants throw; `Driver.Domain` and `Identity.Domain` don't need it and don't have it.
