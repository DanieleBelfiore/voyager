...
public class AuthenticateUserUseCaseTests
{
  ...
  {
    _useCase = new AuthenticateUserUseCase(_repository, _passwordHasher);
  }
  ...
  public async Task Handle_ReturnsSuccess_WhenCredentialsValid()
  {
    ...
    // Act
    var result = await _useCase.Handle(new AuthenticateUser { Username = "ada@example.com", Password = "Str0ng!Pass" }, CancellationToken.None);
  ...
  public async Task Handle_ReturnsFailure_WhenUserNotFound()
  {
    ...
    // Act
    var result = await _useCase.Handle(new AuthenticateUser { Username = "missing@example.com", Password = "x" }, CancellationToken.None);
  ...
  public async Task Handle_ReturnsFailure_WhenPasswordInvalid()
  {
    ...
    // Act
    var result = await _useCase.Handle(new AuthenticateUser { Username = "ada@example.com", Password = "wrong" }, CancellationToken.None);
