...
public class RateRideUseCaseTests
{
  ...
  public async Task Handle_UpdatesRiderRatingAndPublishesEvent_WhenCalledByDriver()
  {
    ...
    // Act
    await _useCase.Handle(new RateRide { RideId = ride.Id, Rating = 4, CallerId = driverId }, CancellationToken.None);
  ...
  public async Task Handle_Throws_Unauthorized_WhenCallerIsNotDriver()
  {
    ...
    _repository.GetByIdAsync(ride.Id, Arg.Any<CancellationToken>()).Returns(ride);
    var act = () => _useCase.Handle(new RateRide { RideId = ride.Id, Rating = 4, CallerId = ride.UserId }, CancellationToken.None);
  ...
  public async Task Handle_Throws_WhenRideIsNotCompleted()
  {
    ...
    var ride = new RideEntity(userId, driverId, SomePoint, SomePoint);
    _repository.GetByIdAsync(ride.Id, Arg.Any<CancellationToken>()).Returns(ride);
    var act = () => _useCase.Handle(new RateRide { RideId = ride.Id, Rating = 5, CallerId = driverId }, CancellationToken.None);
  ...
  public async Task Handle_Throws_WhenRatingIsOutOfRange(int rating)
  {
    ...
    var ride = CompletedRide(userId, driverId);
    _repository.GetByIdAsync(ride.Id, Arg.Any<CancellationToken>()).Returns(ride);
    var act = () => _useCase.Handle(new RateRide { RideId = ride.Id, Rating = rating, CallerId = driverId }, CancellationToken.None);
  ...
  public async Task Handle_Throws_WhenRideWasAlreadyRated()
  {
    ...
    var ride = CompletedRide(userId, driverId);
    _repository.GetByIdAsync(ride.Id, Arg.Any<CancellationToken>()).Returns(ride);
    await _useCase.Handle(new RateRide { RideId = ride.Id, Rating = 5, CallerId = driverId }, CancellationToken.None);
