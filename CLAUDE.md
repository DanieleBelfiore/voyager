...
- **Never add SQLite to this repo.** Not as a test provider, not as `Microsoft.EntityFrameworkCore.Sqlite`, not as a `SqliteConnection("DataSource=:memory:")` fixture. It used to be here to unit-test handlers using relational-only EF features (`ExecuteUpdate`/`ExecuteDelete`, raw SQL) that the in-memory provider cannot run, and it was removed on purpose: it proved the logic but never that SQL Server translates the same expression, so it bought confidence it could not back. Those handlers are either covered by the integration suites against real SQL Server, or not unit-tested at all — both are acceptable, reintroducing SQLite is not.
