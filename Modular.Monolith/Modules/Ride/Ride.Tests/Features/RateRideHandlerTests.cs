...
using NSubstitute;
using Ride.Module.Features.RateRide;
...
public class RateRideHandlerTests
{
  ...
  public async Task Handle_UpdatesRiderRatingAndPublishesEvent_WhenCalledByDriver()
  {
    ...
    // Act
    await handler.Handle(new RateRide { RideId = ride.Id, Rating = 4, CallerId = ride.DriverId }, CancellationToken.None);
  ...
  public async Task Handle_Throws_WhenRideNotFound()
  {
    // Arrange
    await using var db = NewContext();
    var handler = new RateRideHandler(db, Substitute.For<IHikyaku>());
