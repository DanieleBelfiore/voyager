...
public class RegisterUserHandlerTests
{
  ...
  [Fact]
  public async Task Register_ShouldNotifyDriverModule_WhenIsDriver()
  {
    await using var db = NewContext();
  ...
  {
    // Identity and Driver own separate DbContexts even in one process, so there is no ambient
    // transaction across the two writes. Leaving the user behind made the failure permanent: the
    // caller was told registration failed, but retrying hit "already_exist" and the driver was
    // never matchable.
    await using var db = NewContext();
    var mediator = Substitute.For<IHikyaku>();
    mediator.Send(Arg.Any<AddDriver>(), Arg.Any<CancellationToken>())
      .Returns<object>(_ => throw new TimeoutException("driver module unavailable"));
    var handler = new RegisterUserHandler(db, new PasswordHasher<object>(), mediator);
